...
class ECLSRec(SequentialRecommender):
    r"""
    ...
    def __init__(self, config, dataset):
        super(ECLSRec, self).__init__(config, dataset)
    ...
    def forward(self, item_seq, item_seq_len, perturbed=0):
        ...
        output = trm_output[-1]
        if perturbed == 1:
            random_noise = torch.rand_like(output, device=output.device)
            output = output + torch.sign(output) * F.normalize(random_noise, dim=-1) * self.eps
        if perturbed == 2:
            random_noise = torch.rand_like(output, device=output.device)
            output = output - torch.sign(output) * F.normalize(random_noise, dim=-1) * self.eps
    ...
    def calculate_loss(self, interaction):
        ...
        seq_output1 = self.forward(item_seq, item_seq_len, 1)
        seq_output2 = self.forward(sem_aug, sem_aug_lengths, 2)

        nce_logits, nce_labels = self.info_nce(seq_output1, seq_output2, temp=self.tau, batch_size=item_seq_len.shape[0],
                                               sim=self.sim)
